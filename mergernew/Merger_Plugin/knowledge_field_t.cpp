...
string temporal_interval_t::to_string(const knowledge_field_t &kf) const {
  ...
  res += "АТРИБУТЫ\n";
  if (begin_condition)
    res += "  Усл: " + begin_condition->to_string(kf, false) + "\n";
  if (end_condition)
    res += "  Усл: " + end_condition->to_string(kf, false) + "\n";
...
string temporal_object_t::loadLK(std::istream& in, string NameLK, int ko) {
    id = ko;
    name = NameLK;
    string trh, name;
    in >> trh;
...
void condition_attr_value_const_t::loadLK(int oid, int aid, string op_, string value_) {
    op = op_;
    obj_id = oid;
    attr_id = aid;
    value = value_;
...
void temporal_interval_t::loadLK(string nameLK, int ki, int obj_id, int attr_id, string op, string value, int end_obj_id, int end_attr_id, string end_op, string end_value) {
    id = ki;
    name = nameLK;
    condition_attr_value_const_t* condition = new condition_attr_value_const_t;
    condition->loadLK(obj_id, attr_id, op, value);
    begin_condition = condition;
    condition_attr_value_const_t* condition1 = new condition_attr_value_const_t;
    condition1->loadLK(end_obj_id, end_attr_id, end_op, end_value);
    end_condition = condition1;
}

string knowledge_field_t::get_obj_name(string condition_str) {
        size_t n = condition_str.find('.');
        string condition_str_obj_name = (n != string::npos) ? condition_str.substr(0, n) : "";
        return condition_str_obj_name;
     }

string knowledge_field_t::get_attr_name(string condition_str) {
    size_t n = condition_str.find('.');
    size_t n1 = condition_str.find('>');
    if (n1 == -1) {
        n1 = condition_str.find('<');}
    if (n1 == -1) {
        n1 = condition_str.find('=');
    }
    size_t n2 = n1 - n;
    string condition_str_attr_name = (n1 != string::npos) ? condition_str.substr(n + 1, n2 - 1) : "";
    return condition_str_attr_name;
}

string knowledge_field_t::get_value(string condition_str) {
    size_t n3 = condition_str.find('=');
    string condition_str_value = (n3 != string::npos) ? condition_str.substr(n3 + 1, 100) : "";
    return condition_str_value;
}

string knowledge_field_t::get_op(string condition_str) {
    size_t n, n1;
    if ( (n = condition_str.find('<')) == -1) {
        if ( (n = condition_str.find('>')) == -1) {
            return "=";
        }   else {
            if ((n = condition_str.find('=')) == -1){
...
void knowledge_field_t::load(std::istream& in) {
  ...
  while (!in.eof()) {
    ...
    if (type == "temporal_object_t") {
      ...
      {
          NameLK.erase(std::remove(NameLK.begin(), NameLK.end(), chars[i]), NameLK.end());
      }
      in >> trh;
      in >> type;
      if (type == "\"Объект\"") {
        temporal_object_t* objectLK = new temporal_object_t(0);
        type = objectLK->loadLK(in, NameLK, ko);
        objects.push_back(objectLK);
        ko = ko + 1;
        mp_objects[objectLK->get_name()] = objectLK;
      } else if (type == "\"Интервал\"") {
          temporal_interval_t* intervalLK = new temporal_interval_t(0);
          string begin_condition_str, end_condition_str, trh, mrk;
          in >> trh >> mrk;
          if (mrk == "Усл:" || mrk == "УслНач:"){
              std::getline(in, begin_condition_str, '\n');
              char chars[] = "()";
              for (unsigned int i = 0; i < strlen(chars); ++i)
              {
                  begin_condition_str.erase(std::remove(begin_condition_str.begin(), begin_condition_str.end(), chars[i]), begin_condition_str.end());
              }
          begin_condition_str.erase(0, 1);
          string begin_condition_str_obj_name = get_obj_name(begin_condition_str);
          int begin_obj_id = get_object_id_by_name(begin_condition_str_obj_name);
          int begin_attr_id = objects[begin_obj_id]->get_attr_id_by_name(get_attr_name(begin_condition_str));
          string value = get_value(begin_condition_str);
          string op = get_op(begin_condition_str);
        //  in >> trh;
          in >> mrk;
          if (mrk == "Усл:" || mrk == "УслОконч:"){
              std::getline(in, end_condition_str, '\n');
              char chars[] = "\"";
              for (unsigned int i = 0; i < strlen(chars); ++i)
              {
                  end_condition_str.erase(std::remove(end_condition_str.begin(), end_condition_str.end(), chars[i]), end_condition_str.end());
              }
          end_condition_str.erase(0, 1);
          string end_condition_str_obj_name = get_obj_name(end_condition_str);
          int end_obj_id = get_object_id_by_name(end_condition_str_obj_name);
          int end_attr_id = objects[end_obj_id]->get_attr_id_by_name(get_attr_name(end_condition_str));
          string value1 = get_value(end_condition_str);
          string op1 = get_op(end_condition_str);
          intervalLK->loadLK(NameLK, ki, begin_obj_id, begin_attr_id, op, value, end_obj_id, end_attr_id, op1, value1);
          intervals.push_back(intervalLK);
          mp_intervals[intervalLK->get_name()] = intervalLK;
          ki = ki + 1;
      }
    } else {
