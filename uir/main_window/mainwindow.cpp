...
DataMiningWindow::DataMiningWindow(QWidget *parent) :
  ...
  ui->setupUi(this);
  sWindow = new dw_create_window();
  DIR *dir;
  struct dirent *ent;
  if ((dir = opendir ("f:\\StudyMephi\\UIR\\uir\\build-main_window-Desktop_Qt_5_13_2_MinGW_64_bit-Debug\\")) != NULL) {
    while ((ent = readdir (dir)) != NULL) {
       {
            if ( strstr( ent->d_name, ".sqlite" ) != 0 )
            {
                ui->comboBox->addItem(ent->d_name);
            }
        }
    }
    closedir (dir);
...
void DataMiningWindow::on_open_db_button_clicked() {
  std::string database_name = ui->comboBox->currentText().toStdString();
  if (database_name == "") {
...
void DataMiningWindow::on_dw_create_button_clicked()  {
    sWindow->show();
}
